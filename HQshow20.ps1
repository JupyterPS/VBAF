...
class Show20 : BaseShow {
    ...
    hidden [void] RandomizeCastle($castle, [int]$canvasWidth) {
        ...
        # Weighted types
        $types = @(
        "Gothic", "FairyTale", "Cathedral", "Oriental", "Palace"
        "Fortress", "Palace", "Cathedral", "Oriental", "Palace" 
        "Wizard", "Cathedral", "FairyTale", "Oriental", "Palace"
    ...
    "FairyTale",  flasker(spids hat)        "Spire"
...
#>
    ...
    hidden [void] BuildFortressCastle($castle, [int]$groundY) {
        ...
        for ($t = 0; $t -lt 5; $t++) {
            ...
            $towers += @{
                X = $tx; Width = $tw; Height = $th
                HasRoof = $true; RoofType = "Battlement"
