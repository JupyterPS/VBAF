...
$listBox.Height = 135
                 ...
                 {
                      $match = $array[$x]
                      foreach($line in Get-Content -Path "$basePath\FO_ELForbrugÅret.txt")                      
                      {
                           if ($line.Substring(7,10) -match $match)
                           {  
                               $myArray += ($Line.Substring(26,5))
                           }
                      }
                 $x++
                 }
                 ...
                 ############################################################################################################
                 Invoke-expression -Command $basePath\FO_EL3ÅrsforbrugChart1.ps1
                 ############################################################################################################
